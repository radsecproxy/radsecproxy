...
void free_match_srvconf(struct clsrvconf *conf) {
    free(conf->matchcertattr);
    conf->matchcertattr = NULL;
    if (conf->certcnregex)
        regfree(conf->certcnregex);
    free(conf->certcnregex);
    conf->certcnregex = NULL;
    if (conf->certuriregex)
        regfree(conf->certuriregex);
    free(conf->certuriregex);
    conf->certuriregex = NULL;
    if (conf->certdnsregex)
        regfree(conf->certdnsregex);
    free(conf->certdnsregex);
    conf->certdnsregex = NULL;
    memset(&conf->certipmatch, 0, sizeof(struct in6_addr));
    conf->certipmatchaf=0;
}
...
-----END CERTIFICATE-----");
    ...
    /* test explicit CN regex */
    {
        conf.name = "test";
        conf.certnamecheck = 0;
        conf.matchcertattr = stringcopy("CN:/t..t/",0);

        ok(1,addmatchcertattr(&conf),"explicit cn regex config");
    ...
    /* test explicit ip match to SAN IP */
    {
        conf.name = "test";
        conf.certnamecheck = 0;
        conf.matchcertattr = stringcopy("SubjectAltName:IP:192.0.2.1",0);

        ok(1,addmatchcertattr(&conf),"explicit san ip config");
    ...
    /* test explicit ipv6 match to SAN IP */
    {
        ...
        conf.name = "test";
        conf.certnamecheck = 0;
        conf.matchcertattr = stringcopy("SubjectAltName:IP:2001:db8::1",0);

        ok(1,addmatchcertattr(&conf),"explicit san ipv6 config");
    ...
    /* test explicit SAN DNS regex */
    {
        conf.name = "test";
        conf.certnamecheck = 0;
        conf.matchcertattr = stringcopy("SubjectAltName:DNS:/t..t\\.local/",0);

        ok(1,addmatchcertattr(&conf),"explicit san dns regex config");
    ...
    /* test explicit SAN URI regex */
    {
        conf.name = "test";
        conf.certnamecheck = 0;
        conf.matchcertattr = stringcopy("SubjectAltName:URI:/https:\\/\\/test.local\\/profile#me/",0);
    ...
    /* test valid config syntax */
    {
        ...
        ok(1,addmatchcertattr(&conf),"test regex config syntax");
    ...
    /* test invalid config syntax */
    {
        ...
        conf.matchcertattr = stringcopy("CN:t..t",0);
        ok(0,addmatchcertattr(&conf),"test invalid syntax regex");
        free_match_srvconf(&conf);

        conf.matchcertattr = stringcopy("SAN:/t..t/",0);
        ok(0,addmatchcertattr(&conf),"test invalid syntax attribute");
        free_match_srvconf(&conf);

        conf.matchcertattr = stringcopy("SubjectAltName:IP:1.2.3",0);
        ok(0,addmatchcertattr(&conf),"test invalid syntax ip");
        free_match_srvconf(&conf);

        conf.matchcertattr = stringcopy("SubjectAltName:IP:2001:db8:1",0);
        ok(0,addmatchcertattr(&conf),"test invalid syntax ipv6");
        free_match_srvconf(&conf);
    }
    ...
    /* test explicit & implicit combined */
    {
        ...
        ok(1,addmatchcertattr(&conf),"combined config");
